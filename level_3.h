...
void timer3() {
	...
	if (time == 1000) {//다시 실행될시에는 이게 작동을 안한다
		bart_detect3();
		printf("%d", time);
...
void AI3(int *Ax3, int *Ay3, int *direction3, int *confirm_U3, int *confirm_D3, int *confirm_R3, int *confirm_L3) {
	int tmpx = 0;
	int tmpy = 0;
	...
	if (cnt_ai3 % 3000 == 0) {
		if (totalmap3[*Ay3][*Ax3] != 1) {
			...
			else if (*direction3 == 2) {//아래 2
				++*Ay3;
			}
			else if (*direction3 == 3) {//왼 3
				--*Ax3;
			}
			else if (*direction3 == 4) {//오른 4
				++*Ax3;
			}
		...
		if (totalmap3[*Ay3][*Ax3] == 1) {
			...
			else if ((select > 2 && select <= 4) && *confirm_D3 != 1) {//아래 2
				*direction3 = 2;
				//재설정
				*confirm_U3 = 0;
				*confirm_D3 = 1;
			...
			else if ((select > 4 && select <= 6) && *confirm_L3 != 1) {//왼 3
				*direction3 = 3;
				//재설정
				*confirm_U3 = 0;
				*confirm_D3 = 0;
			...
			else if ((select > 6 && select <= 8) && *confirm_R3 != 1) {//오른 4
				*direction3 = 4;
				//재설정
				*confirm_U3 = 0;
				*confirm_D3 = 0;
